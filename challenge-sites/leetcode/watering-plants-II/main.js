...
and two integers capacityA and capacityB representing the capacities of
Alice's and Bob's watering cans respectively,
...
/* Observations
    - The capacities can be different
    - Each capacity is enough to water each plant, so Alice and Bob don't need
        to overlap (other than potentially converging in the middle)
    - We only need to care about alice and bob overlapping in the middle of
    arrays with odd-numbered lengths
*/
...
var minimumRefill = function(plants, capacityA, capacityB) {
    var count = 0;
    var skipAlice = false, skipBob = false;
    var aliceCap = capacityA;
    var bobCap = capacityB;
    const oddPlants = plants.length % 2 != 0;
    const midpoint = Math.ceil(plants.length / 2) - 1;
    ...
    for(var i = 0; i <= midpoint; i++) {
        ...
        if(!skipBob) {
            console.log("Bob");
            var bobTarget = plants[plants.length - i - 1];
            //refill?
            if(bobCap < bobTarget) {
                count++;
                bobCap = capacityB;
